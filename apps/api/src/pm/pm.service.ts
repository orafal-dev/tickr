...
const defaultStatusRows: ReadonlyArray<
...
export class PmService {
  ...
  private async resolveDefaultStatusId(
    ...
    if (!id) {
      throw new BadRequestException('No workflow statuses exist for this organization.');
    }
    return id;
  }
  ...
  private async assertLabelsInOrg(
    ...
    if (count !== labelIds.length) {
      throw new BadRequestException('One or more labels are invalid for this organization.');
    }
  }
